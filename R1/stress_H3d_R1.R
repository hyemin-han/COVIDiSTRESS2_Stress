...
# PSS
# no random 
model.mediator.n <- bf(sps ~ identity+ gender + education + work_location + age+
                         SSS_faml+ relationship_status)
model.pss.n <- bf(pss ~ identity+ sps+gender + education + work_location + age+
                    SSS_faml+ relationship_status)

med.pss.n = brm(
  model.mediator.n + model.pss.n + set_rescor(F),
  data=data.filtered,
  family = gaussian(),
  cores=4,chains=4, save_pars = save_pars(all = T),
  sample_prior ='yes', seed=1660415,prior=prior.coef
...
# testing
hypothesis(med.pss.n, 'sps_identity>0') # Inf
hypothesis(med.pss.n, 'pss_identity<0') # 0 -> full mediation?
hypothesis(med.pss.n, 'pss_sps<0') # Inf

## RES
model.res.n <- bf(resilience ~ identity+ sps+gender + education + work_location + age+
                    SSS_faml+ relationship_status)

med.res.n = brm(
  model.mediator.n + model.res.n + set_rescor(F),
  data=data.filtered,
  family = gaussian(),
  cores=4,chains=4, save_pars = save_pars(all = T),
  sample_prior ='yes', seed=1660415,prior=prior.coef
